...
int SG_sendLogin(int sockfd, char *drt, char *passhash, char *funcao)
{
	...
	if(send(sockfd, lineToSend, strlen(lineToSend), 0) == -1){
		log_write(LOG_CLIENT_FILE, "ERRO: wellcome send() [%s] for [%s]\n", strerror(errno), drt);
		return(NOK);
	}
...
int SG_sendExit(int sockfd, char *drt, char *funcao)
{
	...
	if(send(sockfd, lineToSend, strlen(lineToSend), 0) == -1){
		log_write(LOG_CLIENT_FILE, "ERRO: send() exit [%s]\n", strerror(errno));
...
{
	char confirmaEnvio[CONFIRMA_ENFIO_LEN] = {'\0'};
	...
	while(1){
		...
		do{
			memset(confirmaEnvio, '\0', CONFIRMA_ENFIO_LEN);
		...
		if(confirmaEnvio[0] == 's' || confirmaEnvio[0] == 'S'){
			...
			log_write(LOG_CLIENT_FILE, "Mensagem [%s] enviada.\n", lineToSend);
							 
			if(send(socket, lineToSend, strlen(lineToSend), 0) == -1){
				log_write(LOG_CLIENT_FILE, "ERRO: send() [%s] [%s]\n", lineToSend, strerror(errno));
...
int geraArqDRTs(void)
{
	...
	fprintf(f, "22222-%s\n", STR_OPERMAQUINA);
	fprintf(f, "33333-%s\n", STR_SUPMAQUINA);
	fprintf(f, "\n\nLembrar: O arquivo de DRTs deve obrigatoriamente ter o nome [%s] e a funcao nao deve ter mais de [%d] caracteres!", DRT_FILE, VALOR_FUNCAO_LEN);
...
int SG_relacionaDRTTipoUsuario(char *drt, char *funcao, tipoUsuario_t *usrType)
{
	...
	if((fDRT = fopen(DRT_FILE, "r")) == NULL){
		log_write(LOG_CLIENT_FILE, "Unable to open/read [%s]! [%s]\n", DRT_FILE, strerror(errno));
		return(NOK);
	}
	...
	while(!feof(fDRT)){
		...
		if(fgets(line, LINE_LEN, fDRT) == NULL) break;
		...
		if(strncmp(drtReaded, drt, DRT_LEN) == 0){
			...
			if(strcmp(funcao, STR_FORNOELETRICO) == 0){
				...
				*usrType = UNDEFINED_USER;
				log_write(LOG_CLIENT_FILE, "DRT [%s] cadastrada mas funcao nao definida!\n", drt);
				fclose(fDRT);
				return(NOK);
			}
		}
	...
	if(feof(fDRT)){
		log_write(LOG_CLIENT_FILE, "DRT [%s] nao cadastrada neste sistema!\n", drt);
		fclose(fDRT);
		return(NOK);
	}
...
int SG_fazerLogin(char *drt, char *passhash, char *funcao, tipoUsuario_t *userType)
{
	...
	if(SG_relacionaDRTTipoUsuario(drt, funcao, userType) == NOK)
		log_write(LOG_CLIENT_FILE, "Tentativa de acesso com DRT [%s] nao reconhecida ou com funcao cadastrada invalida as [%s]\n", drt, time_DDMMYYhhmmss());
