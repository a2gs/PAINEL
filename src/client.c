...
int main(int argc, char *argv[])
{
	...
	for(;;){
		...
		if(BL_fazerLogin(id, passhash, level, &usrType) == PAINEL_NOK){
		...
		if(BL_sendLogin(sockfd, id, passhash, level) == PAINEL_NOK){
			logWrite(&log, LOGOPALERT, "The user was not recognized on the server: [%s][%s][%s]!\n", id, passhash, level);
			printf("Usuario, funcao ou senha invalidos!\n");
			continue;
		}

		BL_clientScreen(sockfd, id, level, usrType);

		if(BL_sendLogoutExit(sockfd, id, level) == PAINEL_NOK){
